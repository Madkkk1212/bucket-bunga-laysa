...
│                                                  │
├─ STEP 2: Select Wrapper Paper Type              │
│  (Kraft, Tissue, Glossy, Matte, Crinkle, etc)  │
│                                                  │
├─ STEP 3: Select & Arrange Flowers (30 options)  │
│  (User can click multiple times to add flowers) │
│                                                  │
├─ STEP 4: Preview Bucket (2D Display)            │
│  (Real-time visual preview)                     │
│                                                  │
├─ STEP 5: Customize Text/Ucapan                  │
│  (Input text, font, size, color, position)     │
│                                                  │
├─ STEP 6: Download as PNG/JPG                    │
...
```javascript
// Pre-load flower images
const flowerImages = {};
flowers.forEach(flower => {
  const img = new Image();
  img.src = flower.image_url;
  flowerImages[flower.id] = img;
});

// Render flowers dengan tangkai di dalam bucket
const renderFlowers = (ctx, selectedFlowers, bucketDimensions) => {
  // Calculate bucket mouth position (center top area)
  const bucketMouthX = canvas.width / 2;
  const bucketMouthY = 250; // dari top canvas
  
  selectedFlowers.forEach((flowerData, index) => {
    const flower = flowerImages[flowerData.flower_id];
    
    if (flower && flower.complete) {
      // Calculate position dengan slight rotation untuk natural look
      const angle = (index / selectedFlowers.length) * Math.PI * 2; // Circular distribution
      const radius = 60; // Spread radius di mulut bucket
      
      const x = bucketMouthX + Math.cos(angle) * radius - (flower.width / 2);
      const y = bucketMouthY - flower.height; // Anchor dari bottom
      
      ctx.save();
      ctx.translate(x + flower.width/2, y + flower.height);
      ctx.rotate((Math.random() - 0.5) * 0.3); // Slight random rotation
      ctx.drawImage(flower, -flower.width/2, -flower.height, flower.width, flower.height);
      ctx.restore();
    }
  });
};
...
**Flower Positioning Strategy:**
```
        🌺 🌺 🌺
      🌺   🌺   🌺
    🌺           🌺
  ╔════════════╗  ← Bucket mouth (anchor points)
  ║            ║
  ║   BUCKET   ║
  ║            ║
  ╚════════════╝
  
Bunga tersusun circular dari center dengan natural overlap
```
...
├─ Radio Buttons / Cards:
│  ├─ ○ Small (15cm)
│  ├─ ○ Medium (25cm) [default selected]
│  ├─ ○ Large (35cm)
│  └─ ○ XL (45cm)
└─ Bottom: [← Back] [Next →]

Step 2: Wrapper Paper
├─ Title: "Choose Wrapper Paper"
├─ Description: "Select your preferred wrapper style"
├─ Grid (2 columns) of paper types with color preview:
...
├─ Subtitle: "Flowers (5/25 selected)"
├─ Search/Filter: [Search by name]
├─ Grid (3 columns) of 30 flowers:
│  └─ [Flower thumbnail] Flower Name
│     └─ Clickable to add
├─ Added flowers list (below grid):
│  ├─ Rose Red (×3) [remove]
│  ├─ Tulip Yellow (×2) [remove]
│  └─ etc.
└─ Bottom: [← Back] [Next →]

Step 4: Preview Bucket
├─ Title: "Preview Your Bucket"
├─ Message: "Your bucket looks amazing! 🎉"
├─ Tip: "Proceed to add text or download now"
└─ Bottom: [← Back] [Customize Text] [Download]
...
├─ Color:
│  ├─ Label: "Color"
│  └─ Color Picker: [■ Black] or palettes
├─ Position:
│  ├─ Label: "Position"
...
│  ├─ Label: "Opacity"
│  └─ Slider: [■──────●] 100%
└─ Bottom: [← Back] [Review] [Download]

Step 6: Download
├─ Title: "Download Your Design"
├─ Message: "Your bucket design is ready!"
├─ Format Selection:
...
│  ├─ [Download PNG]
│  └─ [Share Design]
└─ Success Message: "Downloading..." → "Done!"
```
